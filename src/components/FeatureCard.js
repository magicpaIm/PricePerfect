...
const FeatureCard = ({ featurecardData, index, data_aos, data_aos_delay}) => {
    ...
    return (
        <div className={`flex flex-col  rounded-xl w-[30%] px-10 py-10 ${index == 0 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"}`} data-aos={data_aos} data-aos-delay={data_aos_delay}>
            ...
            <div className='flex inline-flex  py-5'>
                <span className='text-6xl font-bold'>{price}</span><span className='self-end text-3xl font-medium'>{priceplan}</span>
            </div>
        </div>
    )
}
